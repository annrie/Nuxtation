...
::alert{type='warning' }
**2026-08 追記：§3 で解説している `server/api/ogp.ts` は、現在このサイトでは使用していません。**

`?url=` で受け取ったURLを検証せずに取得する実装のため、サーバーを伴う構成で公開すると
危険です。現在は OGP をビルド時に解決して JSON に保存し、実行時のエンドポイントは
廃止しています。

以下の手順をそのまま採用する場合は §3 の追記を先に読んでください。
::
...
.link-card-url {
  font-size: 0.6875rem;
  /* 不透明度を掛けると白背景で 3.28:1 まで落ち WCAG AA(4.5:1) を割る。
     素の gray-500 なら 4.83:1。 */
...
.dark .link-card-url {
  /* 明側と同じ理由で不透明度なし（素の slate-400 で 6.96:1）。 */
...
::alert{type='warning' }
**この実装をそのまま本番に置かないでください。**

`getQuery(event)` で受けた `url` を検証せずに `open-graph-scraper` へ渡しています。
**サーバーを伴う構成（SSR / Node ランタイム）で公開すると、訪問者が任意のホストへ
サーバーサイドからリクエストを飛ばせます。** 社内ネットワークやクラウドのメタデータ
エンドポイントへ到達させることも可能です。取得は Fetch で行われ、リダイレクトは既定で
追従します（v6 では `fetchOptions.redirect` で制御します）。最初のURLだけ検証しても
追従先で抜けられます。

**推奨は「実行時に取得しない」ことです。** link-card に渡すURLは記事側で確定するので、
ビルド前にまとめて取得してJSONに保存しておけば、実行時のエンドポイント自体が不要になります。
**実行時の攻撃面が無くなり**、表示時のネットワーク往復も省けます。当サイトは現在この方式です。

ただし「攻撃面がゼロになる」わけではありません。取得はビルドマシンへ移るだけで、
記事に書いたURLがリダイレクト先も含めて内部アドレスへ解決すれば、ビルド時にそこへ
リクエストが飛びます。取得対象を自分が書いた記事に限定できる前提が崩れる場合
（外部からのPRを受け付ける等）は、取得スクリプト側にも宛先の検証が要ります。

宛先を検証して実行時取得を続ける道もありますが、許可リスト・プライベートIPの拒否・
リダイレクト追従先の再検証・レート制限を揃えて初めて成立し、その後も回避手法に追随し
続ける必要があります。入力集合が閉じているなら割に合いません。

なお `open-graph-scraper` の `timeout` は**秒単位**で、内部的に1000倍されます。
本記事の初出時は `20000`（ミリ秒のつもり）と書いており、実際には約5.5時間を
意味していました。下のサンプルは `20` に修正済みです。
::
...
// タイムアウト値を設定。open-graph-scraper の timeout は「秒」単位で、
// 内部で1000倍される。20000 と書くと約5.5時間になるので注意。
// この値は必要に応じて調整してください。
const OGS_REQUEST_TIMEOUT = 20; // 秒
...
export default defineEventHandler(async (event) => {
  ...
  try {
    console.log(`[OGP API] Fetching OGP for: ${url} with timeout: ${OGS_REQUEST_TIMEOUT}s`);
    const { result, error: ogsInternalError } = await ogs({
      url,
      timeout: OGS_REQUEST_TIMEOUT, // 秒単位で指定する
      // リダイレクトは Fetch の既定で追従する。制御は fetchOptions 経由で行う。
      // v6 に 'followRedirect' や 'retry' というオプションは存在しない。
      // 受け付けるオプションはバージョンで変わるので、公式の型定義を確認すること。
    });
    ...
    // タイムアウト関連のエラーかどうかを判定。
    // 注意: open-graph-scraper は例外を二重に包む。まず内部で AbortError を
    // new Error('The operation was aborted due to timeout') に詰め替え(name も code も失われる)、
    // さらに入口が Error ではないプレーンオブジェクトを throw する。
    // メッセージは error.result.error、元の例外は error.result.errorDetails に入る。
    // そのため error.name / error.code だけを見ると常に外れ、504 のつもりが 500 になる。
    const ogsMessage = error?.result?.error ?? error?.result?.errorDetails?.message;
    const rawMessage = ogsMessage ?? error?.message;
    if (error?.name === 'AbortError' ||
        (rawMessage && rawMessage.toLowerCase().includes('timeout')) ||
...
::link-card{propsUrl="https://example.com"}
::
