...
`getQuery(event)` で受けた `url` を検証せずに `open-graph-scraper` へ渡しているため、
公開すると任意のホストへサーバーサイドからリクエストを飛ばせます。社内ネットワークや
クラウドのメタデータエンドポイントへ到達させることも可能です。`open-graph-scraper` は
`followRedirect` が既定で有効なので、最初のURLだけ検証してもリダイレクト追従先で抜けられます。

**推奨は「実行時に取得しない」ことです。** link-card に渡すURLは記事側で確定するので、
ビルド前にまとめて取得してJSONに保存しておけば、実行時のエンドポイント自体が不要になります。
攻撃面がゼロになり、表示時のネットワーク往復も無くなります。当サイトは現在この方式です。

宛先を検証して実行時取得を続ける道もありますが、許可リスト・プライベートIPの拒否・
リダイレクト追従先の再検証・レート制限を揃えて初めて成立し、その後も回避手法に追随し
続ける必要があります。入力集合が閉じているなら割に合いません。

なお下のコードの `timeout: 20000` は誤りです。`open-graph-scraper` の `timeout` は
**秒単位**で内部的に1000倍されるため、20000 は約5.5時間を意味します。正しくは `20` です。
::
