...
- **`pnpm peers check` does not prove Vite compatibility.** The override rewrites the `peerDependencies` recorded in `pnpm-lock.yaml`, so a genuinely incompatible package is reported as satisfied — `@nuxt/devtools-kit@2.7.0` declares `vite: ">=6.0"` upstream but appears as `^8.1.5` in the lockfile. Use `pnpm check:vite-peers`, which reads the manifests unpacked under `node_modules/.pnpm` (the override does not rewrite those). **It compares each package against the Vite actually linked beneath its own virtual-store entry, not against every Vite in the tree** — pnpm may legitimately resolve several, and cross-checking all of them fails correctly-linked packages (5 such false positives exist in the current store). It runs in `ssg-check`. Pass `--vite <version>` to try a different version, and to confirm the check can still fail — `--vite 5.0.0` must exit 1.
