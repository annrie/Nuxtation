...
`?url=` で受け取ったURLを検証せずに取得する実装で、公開状態では任意のホストへ
サーバーサイドからリクエストを飛ばせる状態でした。現在は OGP をビルド時に解決して
JSONに保存し、実行時のエンドポイントは廃止しています。
...
エンドポイントへ到達させることも可能です。`open-graph-scraper` は `followRedirect` が
既定で有効なので、最初のURLだけ検証してもリダイレクト追従先で抜けられます。

なお当サイトは `nuxi generate` の静的出力を配信しており、静的出力にサーバーAPIルートは
含まれません。そのため訪問者がこのエンドポイントを叩ける状態ではなく、任意URLの取得が
起きていたのはプリレンダー中のビルドマシン上だけでした。危険性はあくまで
サーバー構成で公開した場合の話です。
